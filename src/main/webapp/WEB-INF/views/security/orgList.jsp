...
<body>
	<form id="mainForm" action="${ctx}/security/org?lookup=${lookup }"
		method="get">
		...
		<table class="table_all" align="center" border="0" cellpadding="0"
			...
			<tr>
				<td class="td_table_1"><span>部门名称：</span></td>
				...
				<td align="left"><c:choose>
						<c:when test="${empty lookup}">
							<shiro:hasPermission name="ORGEDIT">
								<input type='button'
									onclick="addNew('${ctx}/security/org/create')"
		...
		<table class="table_all" align="center" border="0" cellpadding="0"
			cellspacing="0">
			<tr>
				<td align=center width=45% class="td_list_1" nowrap>部门名称</td>
				<td align=center width=45% class="td_list_1" nowrap>上级部门名称</td>
				<td align=center width=10% class="td_list_1" nowrap>操作</td>
			</tr>
			<c:forEach items="${page.result}" var="org">
				<tr>
					<td class="td_list_2" align=left nowrap>${org.name}&nbsp;</td>
					<td class="td_list_2" align=left nowrap>
						${org.parentOrg.name}&nbsp;</td>
					<td class="td_list_2" align=left nowrap><c:choose>
							<c:when test="${empty lookup}">
								<shiro:hasPermission name="ORGDELETE">
									<a href="${ctx}/security/org/delete/${org.id }" class="btnDel"
										title="删除" onclick="return confirmDel();">删除</a>
								</shiro:hasPermission>
								<shiro:hasPermission name="ORGEDIT">
									<a href="${ctx}/security/org/update/${org.id }" class="btnEdit"
										title="编辑">编辑</a>
								</shiro:hasPermission>
								<a href="${ctx}/security/org/view/${org.id }" class="btnView"
									title="查看">查看</a>
							</c:when>
							<c:otherwise>
								<a href="javascript:void(0)" class="btnSelect" title="选择"
									onclick="bringback('${org.id}','${org.name }')">选择</a>
							</c:otherwise>
						</c:choose></td>
				</tr>
			</c:forEach>
