...
<body>
	<form id="mainForm" action="${ctx}/security/role?lookup=${lookup }"
		method="get">
				...
				<td align="left"><c:choose>
						<c:when test="${empty lookup}">
							<shiro:hasPermission name="ROLEEDIT">
								<input type='button'
									onclick="addNew('${ctx}/security/role/create')"
		...
		<table class="table_all" align="center" border="0" cellpadding="0"
			cellspacing="0">
			<tr>
				<td align=center width=45% class="td_list_1" nowrap>角色名称</td>
				<td align=center width=45% class="td_list_1" nowrap>角色描述</td>
					...
					<td class="td_list_2" align=left nowrap><c:choose>
							<c:when test="${empty lookup}">
								<shiro:hasPermission name="ROLEDELETE">
									<a href="${ctx}/security/role/delete/${role.id }"
										class="btnDel" title="删除" onclick="return confirmDel();">删除</a>
								</shiro:hasPermission>
								<shiro:hasPermission name="ROLEEDIT">
									<a href="${ctx}/security/role/update/${role.id }"
										class="btnEdit" title="编辑">编辑</a>
								</shiro:hasPermission>
								<a href="${ctx}/security/role/view/${role.id }" class="btnView"
									title="查看">查看</a>
							</c:when>
							<c:otherwise>
								<a href="javascript:void(0)" class="btnSelect" title="选择"
									onclick="bringback('${role.id}','${role.name }')">选择</a>
							</c:otherwise>
						</c:choose></td>
				</tr>
			</c:forEach>
