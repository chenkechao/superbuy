...
<body>
	<form id="mainForm" action="${ctx}/security/menu?lookup=${lookup }"
		method="get">
				...
				<td align="left"><c:choose>
						<c:when test="${empty lookup}">
							<shiro:hasPermission name="MENUEDIT">
								<input type='button'
									onclick="addNew('${ctx}/security/menu/create')"
		...
		<table class="table_all" align="center" border="0" cellpadding="0"
			cellspacing="0">
			<tr>
				<td align=center width=45% class="td_list_1" nowrap>菜单名称</td>
				<td align=center width=45% class="td_list_1" nowrap>上级菜单名称</td>
				<td align=center width=10% class="td_list_1" nowrap>操作</td>
			</tr>
			<c:forEach items="${page.result}" var="menu">
				<tr>
					<td class="td_list_2" align=left nowrap>${menu.name}&nbsp;</td>
					<td class="td_list_2" align=left nowrap>
						${menu.parentMenu.name}&nbsp;</td>
					<td class="td_list_2" align=left nowrap><c:choose>
							<c:when test="${empty lookup}">
								<shiro:hasPermission name="MENUDELETE">
									<a href="${ctx}/security/menu/delete/${menu.id }"
										class="btnDel" title="删除" onclick="return confirmDel();">删除</a>
								</shiro:hasPermission>
								<shiro:hasPermission name="MENUEDIT">
									<a href="${ctx}/security/menu/update/${menu.id }"
										class="btnEdit" title="编辑">编辑</a>
								</shiro:hasPermission>
								<a href="${ctx}/security/menu/view/${menu.id }" class="btnView"
									title="查看">查看</a>
							</c:when>
							<c:otherwise>
								<a href="javascript:void(0)" class="btnSelect" title="选择"
									onclick="bringback('${menu.id}','${menu.name }')">选择</a>
							</c:otherwise>
						</c:choose></td>
				</tr>
			</c:forEach>
